...
const FeesDetails = ({
  ...
  // Auto-split: ensure exactly 4 installments and divide netPayable equally
  ...
  const getAutoSplitInstallments = (rows) => {
  ...
  const handleAutoSplit = () => {
    setFormData((prev) => ({
      ...prev,
      installments: getAutoSplitInstallments(prev.installments),
    }));
  };

  const handleAddRow = () => {
    setFormData((prev) => {
      if (prev.installments.length >= 4) {
        return prev;
      }
      const missingCount = 4 - prev.installments.length;
      const newInstallments = [
        ...prev.installments,
        ...Array.from({ length: missingCount }, (_, i) =>
          emptyInstallment(`Installment ${prev.installments.length + i + 1}`, "", 0),
        ),
      ];
      return {
        ...prev,
        installments: getAutoSplitInstallments(newInstallments),
      };
    });
  };
            ...
            >
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    Auto-Split ({installments.length})
                  </button>
                  <button
                    type="button"
